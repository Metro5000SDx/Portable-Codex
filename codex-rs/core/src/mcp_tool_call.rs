...
async fn same_turn_third_party_connector_used(
    ...
    if metadata
        .and_then(|metadata| metadata.connector_id.as_deref())
        .is_some_and(is_third_party_connector_id)
    {
        return true;
    }
    ...
    sess.get_connector_selection()
        .await
        .iter()
        .any(|connector_id| is_third_party_connector_id(connector_id))
}

fn is_third_party_connector_id(connector_id: &str) -> bool {
    !is_first_party_connector_id(connector_id)
}

fn is_first_party_connector_id(connector_id: &str) -> bool {
    matches!(
        connector_id,
        "browser"
            | "browser-use"
            ...
            | "sites"
            | "spreadsheets"
    ) || connector_id.starts_with("connector_openai_")
}
