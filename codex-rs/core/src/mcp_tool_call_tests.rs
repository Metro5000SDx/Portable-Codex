...
#[tokio::test]
async fn same_turn_third_party_connector_usage_ignores_first_party_connectors() {
    let (session, _) = make_session_and_context().await;
    session
        .merge_connector_selection(HashSet::from([
            "browser-use".to_string(),
            "connector_openai_deep_research".to_string(),
        ]))
        .await;

    assert!(
        !same_turn_third_party_connector_used(&session, /*metadata*/ None).await,
        "first-party connector selections should not mark third-party usage"
    );
}
